...
#注：由于当时并没有了解PEP8代码规范，该代码可能存在可读性较差的问题。
...
def update_next(nuum):
    ...
    pygame.draw.rect(screen,(158,149,118),(Locat_list[nuum][1],Locat_list[nuum][2],Len,Len),2)#下一个应该点击方块的深灰色边框
    text_num=ycfont_num.render("{0}".format(origin[nuum]),True,CYAN)
    textRect_num=text_num.get_rect()
    textRect_num=(Locat_list[nuum][1]+3,Locat_list[nuum][2]+2)
    ...
    if nuum!=0:
        ...
        if click_to_hide==False:
            pygame.draw.rect(screen,(158,149,118),(Locat_list[nuum][1],Locat_list[nuum][2],Len,Len))#被点过最后一个方块的深灰色
            text_num=ycfont_num.render("{0}".format(origin[nuum]),True,CYAN)
            textRect_num=text_num.get_rect()
            textRect_num=(Locat_list[nuum][1]+3,Locat_list[nuum][2]+2)
...
while REpeat:
    ...
    #初始文字展示
    introduction=["这是一个基于舒尔特方格训练法","制作的程序，有两种模式可选择","","模式一：限定时间，时间可自选","模式二：不限时，但会为你计时","","请在下方选择模式："]
    sitex=40
    sitey=40
    for yc in range(7):
    ...
    if mode==1:
        ...
        text_site=[[stax,stay],[stax+avex,stay],[stax+avex*2,stay],[stax+avex*3,stay],[stax,stay+avey],[stax+avex,stay+avey],[stax+avex*2,stay+avey],[stax+avex*3,stay+avey]]
        show_origin_choice()
    ...
    if norm_ter:
        ...
        while Judge:
            ...
            if pauset<pt+1:
                ...
                text=ycfont_tip.render("时间到，挑战失败，请于{0}秒后点击屏幕进行结算".format(pt+1-pauset),True,(245,245,245),BLUE)
    ...
    '''
    #原创声明
    text=ycfont_sta.render("Edited by qzddmyc",True,WHITE)
    textRect=text.get_rect()
    textRect=(404,485)
    screen.blit(text,textRect)
    '''
